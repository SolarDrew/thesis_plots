...
dates = ['2011-01-22', '2011-02-01', '2011-02-19'] # AR
coords = [([-200, 200], [300, 700]), ([-600, -200], [-450, -50]), ([-200, 200], [100, 500])] # AR

hfig, hax = plt.subplots(figsize=(16, 12))
for date, coord, label, c in zip(dates, coords, ['$AR_1$', '$AR_2$', '$AR_3$'], ['green', 'red', 'blue'])[2:]:
    d = parse(date)
    ...
    plt.savefig(join(plotshome, 'ar_{:%Y-%m-%dT%H%M}_full').format(parse(qsmap.date)))
    plt.close()
    ...
    plt.title('Active region $EM_{full}$')
    plt.colorbar()
    plt.savefig(join(plotshome, 'arem_{:%Y-%m-%dT%H%M}_full').format(parse(qsmap.date)))
    plt.close()
    ...
    plt.title('Active region DEM width')
    plt.colorbar()
    plt.savefig(join(plotshome, 'arw_{:%Y-%m-%dT%H%M}_full').format(parse(qsmap.date)))
    plt.close()
...
plt.legend()
plt.savefig(join(plotshome, 'ar-histograms-full'))
plt.close()
