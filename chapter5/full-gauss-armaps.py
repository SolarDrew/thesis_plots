...
for date, coord, label, c in zip(dates, coords, ['$AR_1$', '$AR_2$', '$AR_3$'], ['green', 'red', 'blue']):
    ...
    plt.savefig('ar_{:%Y-%m-%dT%H%M}_full'.format(parse(qsmap.date)))
    plt.close()
    ...
    plt.colorbar()
    plt.savefig('arem_{:%Y-%m-%dT%H%M}_full'.format(parse(qsmap.date)))
    plt.close()
    ...
    plt.colorbar()
    plt.savefig('arw_{:%Y-%m-%dT%H%M}_full'.format(parse(qsmap.date)))
    plt.close()
...
plt.legend()
plt.savefig('ar-histograms-full')
plt.close()
